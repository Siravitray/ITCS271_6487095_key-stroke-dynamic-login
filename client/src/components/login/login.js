...
const Login = () => {
    ...
    const captureKeyEvent = (e) => {
        ...
        if (e.type === "keydown") { // if keydown event
            ...
            lastdown = e.timeStamp // set lastdown to current timestamp
            console.log("lastdown ", lastdown);
        } else { // if keyup event
            if (lastdown >= 0) { // if the last key pressed is down
                const dwel = e.timeStamp - lastdown // calculate dwel time
    ...
    useEffect(() => { // add event listeners for key events
        ...
        passwordfeild.onkeydown = captureKeyEvent; // listen for keydown event
        passwordfeild.onkeyup = captureKeyEvent; // listen for keyup event
    ...
    const handleRegister = async(e) => {
        ...
        try {
            ...
            await axios.post("http://localhost:3001/login", user).then((res) => {
                // Extract the user data from the response and display a welcome message
                var user = res.data;
            ...
            if (axios.isAxiosError(err)) {
                console.log({err});
                // If it is, display the error message from the response
    ...
    return (
        <div className="flex flex-col w-full max-w-md px-4 py-8 bg-blue-200 rounded-lg shadow sm:px-6 md:px-8 lg:px-10">
